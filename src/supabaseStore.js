...
export const Store = {
  ...
  async createSquad(name, color) {
    ...
    const [{ data: coach }, { data: admin }] = await Promise.all([
      supabase.from("coaches").select("club_id").eq("id", userId).maybeSingle(),
      supabase.from("platform_admins").select("id").eq("id", userId).maybeSingle(),
    ]);

    let clubId = coach?.club_id;

    if (!clubId && admin) {
      // Platform Admins have no club_id of their own — fall back to
      // whichever club already exists. This only works cleanly while
      // there's exactly one club; once multiple clubs exist, an Admin
      // needs to be asked which club a new squad belongs to, rather
      // than guessing.
      const { data: clubs } = await supabase.from("clubs").select("id").limit(2);
      if (!clubs || clubs.length === 0) {
        throw new Error("No club exists yet — create a club before adding a squad.");
      }
      if (clubs.length > 1) {
        throw new Error("More than one club exists — pick which club this squad belongs to (not yet supported in this screen).");
      }
      clubId = clubs[0].id;
    }

    if (!clubId) {
      throw new Error("Couldn't work out which club this squad belongs to.");
    }
